...
final class AddMovieViewModel: ViewModelType {
    private let addMovieUseCase: MoviesUseCase
    private let navigator: AddMovieNavigator

    init(addMovieUseCase: MoviesUseCase, navigator: AddMovieNavigator) {
    ...
    func transform(input: Input) -> Output {
        ...
        let save = input.saveTrigger.withLatestFrom(titleAndDetails)
                .map { (title, content) in
                    return Movie(body: content, title: title)
                }
                .flatMapLatest { [unowned self] in
                    return self.addMovieUseCase.save(movie: $0)
